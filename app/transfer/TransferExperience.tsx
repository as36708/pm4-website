...
function OkxGuide({ copied, copyText }: { copied: "code" | "reason" | null; copyText: (kind: "code" | "reason", value: string) => Promise<void> }) {
  return <>
    ...
    <GuideBlock number="02" title="查看 OKX 申请表" description="无需自己计算注册日期或交易量。请先登录 OKX,按下方顺序复制邀请码和理由,然后通过橙色按钮打开页面。"><GuideScreenshot src="/guide/okx-step-02.png" alt="OKX 邀请码申请表空白界面" width={780} height={583} /><p className={styles.tip}>看不到邀请码输入框?说明不符合变更条件,直接看第 5 步。</p></GuideBlock>
    ...
    <GuideBlock number="04" title="提交 OKX 申请" description="回到申请页，把刚才复制的推荐码和理由粘贴进去，点「提交」。"><a className={`${styles.outlineButton} ${styles.primaryButton}`} href={OKX_ELIGIBILITY_URL} target="_blank" rel="noopener noreferrer">打开 OKX 申请页 ↗</a><GuideScreenshot src="/guide/okx-step-04.png" alt="OKX 填写推荐码和申请理由后的提交界面" width={795} height={618} /><p className={styles.tip}>填写并点击 Submit 后，仍需等待 OKX 审核批准。</p></GuideBlock>
